...
std::pair<FatJetMatching::FatJetFlavor, const reco::GenParticle*> FatJetMatching::flavorJMAR(const pat::Jet* jet,
  ...
  for (unsigned ipart = 0; ipart<genParticles.size(); ++ipart){
    ...
    }else if (pdgid == ParticleID::p_Z0 || pdgid == ParticleID::p_Zprime0) { // Added Z' to list of acceptable Zs for categorizing pdgID
      // Z
      auto z = getFinal(gp);
      if (isHadronic(z)) {
        if (debug_ || debug_lite){
          using namespace std;
          cout << "jet: " << jet->polarP4() << endl;
        ...
        if (dr_zdaus < genRadius && dr_jet_z < genRadius){ 
          if (debug_ || debug_lite){
            using namespace std;
            cout << "Matching criteria met" << endl;
          }
          return std::make_pair(FatJetFlavor::Z, z);
        }
      }
    }else {
      // ?
